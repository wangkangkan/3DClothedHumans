...
_3D_DATASETS = ['tran_tfrecords_p2_2']


def num_examples(datasets):
    _NUM_TRAIN = {
        'tran_tfrecords_p2_2': 300000,
        'lsp': 1000,
        'lsp_ext': 10000,
        'mpii': 20000,
        'h36m': 312188,
...
class DataLoader(object):
    def __init__(self, config):
        ...
        self.initpara_tag = config.initpara_tag

    def load(self):
        if self.use_3d_label:
            if self.initpara_tag==1:
                image_loader = self.get_loader_w3d_initpara()
            else:
    ...
    def get_loader_w3d_initpara(self):
        """
        Similar to get_loader, but outputs are:
            image_batch: batched images as per data_format
            label_batch: batched keypoint labels N x K x 3
        ...
        fqueue_yes3d = tf.train.string_input_producer(
            files_yes3d, shuffle=do_shuffle, name="input_w3d")
        pointset, label, initparameter = self.read_data_initpara(
            fqueue_yes3d)

        min_after_dequeue = 2000
        capacity = min_after_dequeue + 3 * self.batch_size

        pack_these = [pointset, label, initparameter]
        pack_name = ['pointset', 'label', 'initparameter']
    ...
    def read_data(self, filename_queue):
        with tf.name_scope(None, 'read_data', [filename_queue]):
            ...
            pointset, label = data_utils.parse_example_proto_our_3dpoint(
                example_serialized, self.config.pointNet_pointnum)
    ...
    def read_data_initpara(self, filename_queue):
        with tf.name_scope(None, 'read_data', [filename_queue]):
            reader = tf.TFRecordReader()
            _, example_serialized = reader.read(filename_queue)
            pointset, label, initparameter= data_utils.parse_example_proto_our_3dpoint_initpara(
                example_serialized, self.config.pointNet_pointnum)
            # groundpara = tf.concat([pose, shape, translation], 0)
            # Need to send pose bc image can get flipped.
            # image, label, pose, gt3d = self.image_preprocessing(
            #     image, image_size, label, center, pose=pose, gt3d=gt3d)
            ...
            return pointset, label, initparameter
