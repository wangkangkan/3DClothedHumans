...
import tf_util
from pointnet_util import pointnet_sa_module, pointnet_joints_module
...
def extractfeature_level(point_cloud, point_feat, npoint, radius, nsample, mlp, group_all,scopename, is_training, bn_decay=None):
    ...
    with tf.variable_scope(scopename) as scope:
        ...
        # Note: When using NCHW for layer 2, we see increased GPU memory usage (in TF1.4).
        # So we only use NCHW for layer 1 until this issue can be resolved.
        l1_xyz, l1_points, l1_indices = pointnet_sa_module(l0_xyz, l0_points, npoint, radius, nsample, mlp, mlp2=None, group_all = group_all, is_training=is_training, bn_decay=bn_decay, scope='layer1')
...
def parameter_regression(point_cloud, point_feat, outdim, is_training, bn_decay=None):
    ...
    with tf.variable_scope("regression") as scope:
        ...
        return net, variables
def extract_jointsfeature(point_cloud, point_feat, joints, is_training, bn_decay=None):
    """ Classification PointNet, input is BxNx3, output Bx40 """
    batch_size = point_cloud.get_shape()[0].value
    num_point = point_cloud.get_shape()[1].value
    l0_xyz = point_cloud
    ...
    with tf.variable_scope("extractglobalfeature") as scope:
        ...
        # Note: When using NCHW for layer 2, we see increased GPU memory usage (in TF1.4).
        # So we only use NCHW for layer 1 until this issue can be resolved.
        l1_points, l1_indices = pointnet_joints_module(l0_xyz, l0_points, joints, radius=0.3, nsample=64, mlp=[64,64,128], mlp2=None, group_all=False, is_training=is_training, bn_decay=bn_decay, scope='layer0', use_nchw=True)
...
def extract_globalfeature(point_cloud, point_feat, is_training, bn_decay=None):
    ...
    with tf.variable_scope("extractglobalfeature") as scope:
        ...
        # Note: When using NCHW for layer 2, we see increased GPU memory usage (in TF1.4).
        # So we only use NCHW for layer 1 until this issue can be resolved.
        l1_xyz, l1_points, l1_indices = pointnet_sa_module(l0_xyz, l0_points, npoint=24, radius=0.2, nsample=128, mlp=[64,64,128], mlp2=None, group_all=False, is_training=is_training, bn_decay=bn_decay, scope='layer1', use_nchw=True)
        # l2_xyz, l2_points, l2_indices = pointnet_sa_module(l1_xyz, l1_points, npoint=128, radius=0.4, nsample=64, mlp=[128,128,256], mlp2=None, group_all=False, is_training=is_training, bn_decay=bn_decay, scope='layer2')
        # l3_xyz, l3_points, l3_indices = pointnet_sa_module(l2_xyz, l2_points, npoint=None, radius=None, nsample=None, mlp=[256,512,1024], mlp2=None, group_all=True, is_training=is_training, bn_decay=bn_decay, scope='layer3')
