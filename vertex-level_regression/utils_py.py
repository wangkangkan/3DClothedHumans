...
def subdiv_mesh_x4(mesh):
    ...
    # add vertexes
    crt_id = verts_num
    # edge = np.loadtxt('edge_13055.txt', dtype=int)
    edge = mesh.edge_vertex_indices()
    np.savetxt('edge_20100.txt', edge, "%d")
    ...
    # add faces
    face_edge = mesh.face_vertex_indices()
...
def subdiv_mesh_x4_part(mesh):
    ...
    new_edge = []
    for e in edge:
        dis = cal_distance(verts[e[0]], verts[e[1]])
        if dis > 0.015:
            new_edge.append(e)
    
    np.savetxt('edge_0.01.txt', np.array(new_edge), "%d")
    # for e in mesh.edge_vertex_indices():
    for e in new_edge:
        new_vert_dic[e[0], e[1]] = crt_id
        new_vert_dic[e[1], e[0]] = crt_id
    ...
    for f in mesh.face_vertex_indices():
        ...
        if v4 != 0 and v5 != 0 and v6 != 0:
            ...
            faces_list.append([v6, v5, v3])
            faces_list.append([v4, v5, v6])
        elif v4 != 0 and v5 != 0 and v6 == 0:
            faces_list.append([v1, v4, v3])
            faces_list.append([v4, v2, v5])
            faces_list.append([v4, v5, v3])
        elif v4 != 0 and v5 == 0 and v6 != 0:
            faces_list.append([v1, v4, v6])
            faces_list.append([v4, v2, v6])
            faces_list.append([v6, v2, v3])
        elif v4 == 0 and v5 != 0 and v6 != 0:
            faces_list.append([v1, v2, v6])
            faces_list.append([v6, v2, v5])
            faces_list.append([v6, v5, v3])
        elif v4 == 0 and v5 == 0 and v6 != 0:
            faces_list.append([v1, v2, v6])
            faces_list.append([v6, v2, v3])
            ...
            faces_list.append([v1, v4, v3])
            faces_list.append([v4, v2, v3])
        elif v4 == 0 and v5 != 0 and v6 == 0:
            faces_list.append([v1, v2, v5])
            faces_list.append([v1, v5, v3])
        elif v4 == 0 and v5 == 0 and v6 == 0:
...
if __name__ == "__main__":
    from write2obj import read_obj, write_to_obj
    import glob
    # all_file = sorted(glob.glob('../fittingcode_ourwoman5/resultfemale_9.12/*.obj'))
    # count = 420
    # for f in all_file:
    #     vert, face = read_obj(f)
    #     mesh = make_trimesh(vert, face)
    #     # mesh = flatten_naval(mesh)
    #     # openmesh.write_mesh('./test/flatten.obj', mesh)
    #     # mesh = smpl_detoe(mesh)
    #     # v = np.array(mesh.vertices())
    #     # vv = mesh.points()
    #     # f = mesh.faces()
    #     # write_to_obj('./test/detoe.obj', mesh.vertices(), mesh.faces())
    #     # openmesh.write_mesh('./test/detoe.obj', mesh)
    #     mesh = subdiv_mesh_x4_part(mesh)
    #     openmesh.write_mesh('./data/2x_mesh/{:d}.obj'.format(count), mesh)
    #     count += 1



    vert, face = read_obj('./test_10000/pred_smpl_0.obj')
    mesh = make_trimesh(vert, face)
    edge = mesh.edge_vertex_indices()

    edge = edge[np.lexsort(edge.T[0,None])]

    np.savetxt('edge_for_loss_6890.txt', edge, "%d")
    ...
    np.savetxt('laplance_6890.txt', np.array(con, dtype=int), "%d")