...
_3D_DATASETS = ['tran_tfrecords_30w']
...
class DataLoader(object):
    ...
    def get_loader_w3d(self):
        ...
        pointset, label, para = self.read_data(
            fqueue_yes3d)

        min_after_dequeue = 2000
        capacity = min_after_dequeue + 3 * self.batch_size

        pack_these = [pointset, label, para]
        pack_name = ['pointset', 'label', 'para']
    ...
    def read_data(self, filename_queue):
        with tf.name_scope(None, 'read_data', [filename_queue]):
            ...
            pointset, label, para = data_utils.parse_example_proto_our_3dpoint(
